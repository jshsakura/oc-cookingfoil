...
- ⏳ **M4b** (진행 중, 워커) — 설치 배선(§9.1). M4a stub(network/offline) → 실 `network_util.cpp`/`offline_title_db.cpp` 컴파일 + 자체 얇은 오케스트레이터(HTTPNSP+NSPInstall Prepare/Begin) + `ShopItem→RemoteItem` 어댑터(순수, 테스트) + SDL2 `InstallScreen`(진행바) + shim 라우팅(instPage→화면). [설치]→SD경로(storage=0). **CF-Access 설치 다운로드 주의**: `tin::network`에 커스텀헤더 훅 없음(SetBasicAuth만) → 깨끗이 추가 가능하면 로컬 델타로 hook, 아니면 후속이연(공개/basic 설치는 완성). 엔진 실설치=스위치 빌드검증만, 어댑터+진행화면=호스트검증. **.nro 크기 점프 예상**(설치버튼이 엔진 참조→더 이상 dead-strip 안 됨).
- 다음: **M5** 큐/일괄/NAND/SD토글 실동작 → **M6** oc아이콘·i18n.
